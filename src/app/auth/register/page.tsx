...
// Same orbs + books as login for visual consistency
const ORBS = [
...
export default function RegisterPage() {
  ...
  return (
    <main className="min-h-screen flex">
      ...
      {/* ══ DESKTOP LEFT PANEL — identik dengan login ══ */}
      ...
      <div className="flex-1 flex flex-col bg-white">
        ...
        <div className="lg:hidden px-6 pt-12 pb-6">
          <Link href="/auth/login" className="flex items-center gap-1 text-slate-500 text-sm mb-6 hover:text-slate-700 transition-colors">
            <span className="text-lg">←</span> Kembali
          </Link>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            <Wordmark />
            <h1 className="text-2xl font-bold text-navy-900 mb-1">Selamat Datang</h1>
            <p className="text-slate-500 text-sm">Daftar sekarang untuk meminjam buku</p>
          </motion.div>
        </div>

        {/* Form */}
        <motion.div
          className="flex-1 flex flex-col justify-center px-6 pb-10 lg:px-14 xl:px-20"
          ...
          <div className="w-full max-w-sm mx-auto lg:mx-0">
            ...
            <div className="hidden lg:block mb-8">
              <h2 className="font-serif text-3xl font-black text-navy-900 mb-1">Buat akun</h2>
              <p className="text-slate-500 text-sm">
                Sudah punya akun?{' '}
                <Link href="/auth/login" className="text-navy-700 font-semibold hover:underline">Masuk</Link>
              </p>
            </div>
            ...
            <form onSubmit={handleRegister} className="flex flex-col gap-3">
              <InputField icon={<User className="w-4 h-4" />} type="text" placeholder="Nama"
              ...
              <motion.button type="submit" disabled={loading}
                className="w-full py-3.5 bg-navy-800 text-white rounded-2xl font-semibold text-sm hover:bg-navy-700 disabled:opacity-60 transition-colors mt-1"
                whileTap={{ scale: 0.98 }}>
            ...
            <motion.button onClick={handleGoogle} disabled={loading}
              className="w-full py-3 border border-slate-200 rounded-2xl text-sm font-medium text-slate-700 flex items-center justify-center gap-2 hover:bg-slate-50 transition-all"
              whileTap={{ scale: 0.98 }}>
            ...
            <p className="text-center text-xs text-slate-500 mt-5">
              Sudah memiliki akun?{' '}
              <Link href="/auth/login" className="text-navy-600 font-semibold hover:underline">Masuk</Link>
            </p>
          </div>
        </motion.div>
      </div>
...
const inputCls = `w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl
  text-sm text-slate-800 placeholder-slate-400 outline-none
  focus:border-navy-500 focus:ring-2 focus:ring-navy-200 transition-all`;

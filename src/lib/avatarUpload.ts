...
/**
 * Upload avatar image to Supabase Storage as a draft (does NOT update backend database yet)
 */
export async function uploadAvatarDraft(
  file: File,
  userId: string
): Promise<UploadResult> {
  try {
    ...
    console.error('Avatar draft upload error:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Upload gagal',
    };
